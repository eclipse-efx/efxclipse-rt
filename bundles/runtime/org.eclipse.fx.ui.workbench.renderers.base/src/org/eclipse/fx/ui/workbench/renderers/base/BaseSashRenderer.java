...
public abstract class BaseSashRenderer<N> extends
						...
						if (changedObj instanceof MPartSashContainer) {
							...
							if (BaseSashRenderer.this == parent.getRenderer()) {
								if (UIEvents.isADD(event)) {
									handleChildrenAddition(parent,
											Util.<MPartSashContainerElement>asCollection(event, UIEvents.EventTags.NEW_VALUE));
						...
						if (changedObj instanceof MPartSashContainer) {
							...
							if (BaseSashRenderer.this == parent.getRenderer()) {
								...
								if (UIEvents.EventTypes.SET.equals(eventType)) {
									...
									MUIElement oldValue = (MUIElement) event
											.getProperty(UIEvents.EventTags.OLD_VALUE);
									handleSelectedElement(
											parent,
											(MPartSashContainerElement) oldValue,
											(MPartSashContainerElement) newValue);
								}
							}
						}
					}
				...
				new EventHandler() {
					...
					public void handleEvent(Event event) {
						...
						if (changedObj.isToBeRendered()) {
							...
							if (parent != null) {
								if (BaseSashRenderer.this == parent
										.getRenderer()) {
									MPartSashContainer stack = (MPartSashContainer) parent;
									String eventType = (String) event
											.getProperty(UIEvents.EventTags.TYPE);
									if (UIEvents.EventTypes.SET
											.equals(eventType)) {
	...
	@Override
	public void childRendered(MPartSashContainer parentElement,
			MUIElement element) {
		...
		List<WLayoutedWidget<MPartSashContainerElement>> l = Collections
				.singletonList((WLayoutedWidget<MPartSashContainerElement>) element
						.getWidget());
		sash.addItems(idx, l);
	}
	...
	public void hideChild(MPartSashContainer container, MUIElement changedObj) {
		...
		@SuppressWarnings("unchecked")
		WLayoutedWidget<MPartSashContainerElement> widget = (WLayoutedWidget<MPartSashContainerElement>) changedObj
				.getWidget();
		if (widget != null) {
	...
	void handleChildrenAddition(MPartSashContainer parent,
			Collection<MPartSashContainerElement> elements) {
		Iterator<MPartSashContainerElement> iterator = elements.iterator();
		while (iterator.hasNext()) {
			MPartSashContainerElement element = (MPartSashContainerElement) iterator
					.next();
	...
	void handleChildrenRemove(MPartSashContainer parent,
			Collection<MPartSashContainerElement> elements) {
		Iterator<MPartSashContainerElement> iterator = elements.iterator();
		while (iterator.hasNext()) {
			MPartSashContainerElement element = (MPartSashContainerElement) iterator
	...
	void handleSelectedElement(MPartSashContainer parent,
			MPartSashContainerElement oldElement,
			MPartSashContainerElement newElement) {
