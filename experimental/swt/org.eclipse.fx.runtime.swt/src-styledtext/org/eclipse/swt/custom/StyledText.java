...
//TODO Port it	setData("DEFAULT_DROP_TARGET_EFFECT", new StyledTextDropTargetEffect(this));
}
/**	 
...
void calculateTopIndex(int delta) {
	...
	if (isFixedLineHeight()) {
		...
		topIndex = (int) Math.ceil((float)getVerticalScrollOffset()/verticalIncrement);
...
//TODO	sendAccessibleTextCaretMoved();
}
/**
...
int getPartialBottomIndex() {
	if (isFixedLineHeight()) {
		int lineHeight = renderer.getLineHeight();
		int partialLineCount = (int) Math.ceil((float)clientAreaHeight/ lineHeight);
...
//TODO	sendAccessibleTextChanged(lastTextChangeStart, lastTextChangeNewCharCount, 0);
...
//TODO	sendAccessibleTextChanged(lastTextChangeStart, 0, lastTextChangeReplaceCharCount);
...
//TODO	sendAccessibleTextChanged(0, newCharCount, lastCharCount);
...
// TODO	getAccessible().textSelectionChanged();
	Event event = new Event();
	event.x = selection.x;
	event.y = selection.y;
